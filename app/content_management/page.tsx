        ...
        try {
            ...
            for (const city in area_data) {
                for (const district of area_data[city as keyof typeof area_data]) {
                    const response = await fetch(`api/db/add_city_district?city=${city}&district=${district}`);
                    if (!response.ok) {
                        console.error(`Failed to create city district: ${city} ${district}`);
                    }
                    else{
        ...
        <div className="w-full flex flex-wrap h-screen justify-center items-center overflow-auto">
            <Card className="w-[30%] h-[350px] overflow-auto">
                ...
                <CardContent className="flex flex-wrap w-full justify-center">
                    <Input name="city" placeholder="City" value={form.city} onChange={handleChange} className="m-2" />
                    <Input name="district" placeholder="District" value={form.district} onChange={handleChange} className="m-2"/>
                    <Button onClick={handleOnClick} className="m-2">新增</Button>
                    <Button onClick={addAllCityDistrict} className="m-2">新增全部</Button>
                </CardContent>
            </Card>
        </div>
    )
